...
const validateForm = () => {
    ...
    if (!form.name?.trim()) {
        errors.name = "Defect Type name is required";
    } else if (form.name.trim().length < 2) {
        errors.name = "Defect Type name must be at least 2 characters";
    } else if (form.name.trim().length > 255) {
        errors.name = "Defect Type name must not exceed 255 characters";
    }
...
const fetchDefectType = async () => {
    ...
    try {
        const url = route("defect_types.show.api", props.slug);
        const response = await fetch(url);

        if (!response.ok) {
            throw new Error(
                `HTTP ${response.status}: Failed to fetch defect type`
            );
        }
        ...
        emit("operation", {
            ...
            action: "fetch",
            data: res.data,
            message: "Defect Type data loaded successfully",
        });
    } catch (error) {
        ...
        emit("operation", {
            ...
            action: "fetch",
            error,
            message: `Failed to fetch defect type data: ${error.message}`,
        });

        // Show error toast
        errorToast(`Failed to fetch defect type data: ${error.message}`);
...
const handleRefresh = async () => {
    ...
    successToast("Defect Type data refreshed!");
};
...
const handleSubmit = async () => {
    ...
    const result = await editConfirmDialog("this defect type");
    if (!result.isConfirmed) {
        return;
    }
    ...
    form.transform(() => formData).put(
        route("defect_types.update", props.slug),
        {
            preserveState: true,
            preserveScroll: true,
            onSuccess: (response) => {
...
const handleSuccess = (response) => {
    ...
    successToast("Defect Type updated successfully!");
};
...
const handleError = (errors) => {
    ...
    errorToast("Failed to update defect type. Please check your input.");
};
...
const getLoadingStatus = () => {
    if (isFetching.value) return "Loading defect type data...";
    if (isLoading.value) return "Updating defect type...";
    return "";
};
</script>
...
<template>
    <FormModal
        ...
        description="Update the defect type information below and click save to apply changes."
            ...
            <div v-else>
                <div class="mb-2">
                    ...
                    <FormModalInput
                        ...
                        v-model="form.name"
                        type="text"
                        placeholder="Enter defect type name (e.g., Scratches)"
                        maxlength="255"
                        required
                ...
                <!-- Defect Type info display with formatted date -->
                <div
                    v-if="defectType"
                ...
                >
                    <div class="space-y-2 text-gray-600 dark:text-gray-400">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="font-mono">{{ defectType.slug }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="font-medium">Created:</span>
                            <span>{{ formatDate(defectType.created_at) }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="font-medium">Last Updated:</span>
                            <span>{{ formatDate(defectType.updated_at) }}</span>
                        </div>
                    </div>
                </div>
            </div>
